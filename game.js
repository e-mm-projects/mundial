...
function calculateBaseTeamRating(players, formation) {
    ...
    players.slice(0, 11).forEach((p, index) => {
        if (index >= layout.gk[0] && index < layout.gk[1]) {
            ...
            ratings.mid += (p.stats.spd + p.stats.str + p.stats.eng + p.stats.tek) / 4;
            counts.mid++;
...
function setupNavigation() {
    ...
    const backgrounds = {
        ...
        'mail': 'images/posta1.png',            
        'minileague': 'images/miniliga1.jpg',
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            else if (target === 'hall-of-fame') renderHallOfFame();
            else {
...
window.finishMatchReplay = function() {
    ...
    // 1. Připravíme si závěrečné info
    ...
    // 2. NOVINKA: Zobrazíme odměny jen pokud hráč skutečně něco dostal (> 0)
    if (msg.rewards && (msg.rewards.money > 0 || msg.rewards.xp > 0 || msg.rewards.pXp > 0)) {
        finalHtml += `
    ...
    // 3. Vložíme nad všechny komentáře
...
// --- KONTROLA POŠTY Z CLOUDU  ---
window.fetchCloudMail = async function() {
    if (!playerData.uid) return;
    try {
        const dbRef = window.dbRef(window.db);
        const snapshot = await window.dbGet(window.dbChild(dbRef, `mail_queue/${playerData.uid}`));
        ...
        if (snapshot.exists()) {
            ...
            for (const key in mails) {
                ...
                // Zprávu jsme doručili, smažeme ji z cloudu ať ti nechodí dvakrát
                await window.dbSet(window.dbRef(window.db, `mail_queue/${playerData.uid}/${key}`), null);
            }
            
            // Pojistka, ať nemáš v poště milion zpráv
            if (playerData.mail.length > 15) {
                playerData.mail = playerData.mail.slice(0, 15);
            }
            ...
            if (hasNew) {
                ...
                const activeBtn = document.querySelector('.nav-btn.active');
                if (activeBtn && activeBtn.getAttribute('data-target') === 'mail') {
